...
$( "li.propertie" ).mouseover(function() { $(this ).removeClass('inactivepro').addClass('activepro'); }) .mouseout(function() { $( this ).removeClass('activepro').addClass('inactivepro'); });
 });
...
(function(){
  ...
  if ($sliderExclusive.length) {
    ...
    if (nLisEx > 1) {
      ...
      //if ($ventana.width() >= 768 && $ventana.width() < 1024) { 
      ...
      /*
      if (!$ulSliderEx.hasClass('interval')) {
        var autoPlaySliderEx = setInterval(autoPlayEx, 5000); 
        $ulSliderEx.addClass('interval');  
      }
      // Apareciendo la primera imagen
      apareceImagenB($lisSliderEx.eq(0));
      */
