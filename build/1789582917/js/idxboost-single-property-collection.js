(function ($) {
	...
	$.ajax({
		...
		data: JSON.stringify({ 
			...
			limit: ib_property_collection.limit, 
	...
	const addPropertyInformationToMessage = (form, property) => {
		const visibleMessage = $(form).find('textarea[id="visible_comment"]').val();
		const $hiddenMessage = $(form).find('[id="hidden_comment"]');
	
		if (visibleMessage) {
			$hiddenMessage.val(`${visibleMessage} | ${getPropertyInformation(property)}`);
		} else {
			$hiddenMessage.val(getPropertyInformation(property));
		}
	}

	// isEnterpriseReCaptchaEnabled() vive en js/flex-auth-check.js 
	// (se carga globalmente en todo el sitio), expuesta como función global.

	function loadSPForms(property) {
		const IB_SP_FORM_CONTACT = $(".js-ib-sp-contact-form");

		if (IB_SP_FORM_CONTACT.length) {
			IB_SP_FORM_CONTACT.on("submit", async function (event) {
				event.preventDefault();
				contactForm = $(this);
				...
				try {
					const token = await getReCaptchaToken("contact_inquiry");
