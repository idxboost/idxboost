...
function loadBuildingViewType(inventario){
...
function ib_init_script_elastic() {
  ...
  //save alert collection no script
  ib_detail_building.payload.properties= window.response_building.payload.properties;
  ib_detail_building.payload.meta= window.response_building.payload.meta;
  console.log(ib_detail_building)

             jQuery.ajax({
              url: idxboost_collection_params.ajaxUrl,
              method: "POST",
              data: {
                action: "idxboost_collection_building_list",
                building_id: ib_detail_building.payload.cod_building,
                response_data: JSON.stringify(ib_detail_building),
              },
              dataType: "json",
    ...
    if(ib_building_inventory.load_item=='ajax'){
      jQuery('#dataTable-floorplan-grid').DataTable({ "paging": false }).order([3, 'desc']).draw();
                  ...
                  ib_change_view( $('.ib_collection_view').val() ,$('.ib_collection_tab').val() );
                    ...
                    inventorySliderItems = jQuery("#buildingSliderPicture img").length;
                    loadBuildingViewType(inventorySliderItems);
