...
if (!function_exists('idxboost_contact_page_sc')) {
    function idxboost_contact_page_sc($atts, $content = null)
    {
        ...
        if (
            ...
            if (file_exists(IDXBOOST_OVERRIDE_DIR . '/views/shortcode/idxboost_cms_page_contact.php')) {
                include IDXBOOST_OVERRIDE_DIR . '/views/shortcode/idxboost_cms_page_contact.php';
            } else {
                include FLEX_IDX_PATH . '/views/shortcode/idxboost_cms_page_contact.php';
            }
        ...
        switch ($atts["type"]) {
            case "condo":
                ...
                <form method="post" class="ib-search-box-form">
                    ...
                    <label for="c_sale"><input type="radio" name="for" value="sale" id="c_sale" checked><span></span>FOR
                        SALE</label>
                    <label for="c_rent"><input type="radio" name="for" value="rent" id="c_rent"><span></span>FOR
                        RENT</label>
                    <select name="city">
            ...
            case "house":
                ...
                <form method="post" class="ib-search-box-form">
                    ...
                    <label for="h_sale"><input type="radio" name="for" value="sale" id="h_sale" checked><span></span>FOR
                        SALE</label>
                    <label for="h_rent"><input type="radio" name="for" value="rent" id="h_rent"><span></span>FOR
                        RENT</label>
                    <select name="city">
                        <option value="" selected>Search by City</option>
                        <?php foreach ($cities as $city): ?>
...
if (!function_exists('ib_search_filter_sc')) {
    function ib_search_filter_sc($atts, $content = null)
    {
        ...
        if ($idx_v == "1") {
            ...
            if ( !empty($atts["id"]) ) {
                $access_token = flex_idx_get_access_token();
                $curlParams_body = idxboost_remote_request(FLEX_IDX_BASE_URL.'/get/map_search_filter/'.$atts["id"], array(
                        'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                        'redirection' => 10,
                        'httpversion' => '1.1',
                        'headers' => array(
                ...
                $responseParms = @json_decode($curlParams_body , true);
...
if (!function_exists('ib_mini_search_sc')) {
    function ib_mini_search_sc($atts, $content = null)
    {
        ...
        if ($atts['template'] == 'search_condos') {
            ...
            $list_top_building = array_filter($loop_building, function ($item) {
                return $item['top'] == '1';
            });
...
if (!function_exists('flex_idx_terms_conditions_sc')) {
    function flex_idx_terms_conditions_sc($atts, $content = null)
    {
        ...
        $server_output = idxboost_remote_request(FLEX_IDX_API_TERMS_CONDITIONS, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'body'    => $sendParams,
        ));
...
if (!function_exists('flex_idx_buildings_sc')) {
    function flex_idx_buildings_sc($atts, $content = null)
    {
        ...
        $server_output = idxboost_remote_request(FLEX_IDX_API_BUILDINGS_LIST, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'body'    => $sendParams,
        ));
...
if (!function_exists('idx_buildind_collection_history_sc')) {
    function idx_buildind_collection_history_sc($atts)
    {
        ...
        if (is_array($flex_idx_info) && array_key_exists("agent", $flex_idx_info) &&
            is_array($flex_idx_info["agent"]) &&
            array_key_exists("has_building", $flex_idx_info["agent"]) &&
            $flex_idx_info["agent"]["has_building"] !== true
        ) {
            return "";
        }
...
if (!function_exists('flex_idx_search_sc')) {
    function flex_idx_search_sc($atts, $content = null)
    {
        ...
        $server_output_canti = idxboost_remote_request(FLEX_IDX_API_FAVORITES_LIST, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'body'    => $sendParams_count,
        ));
...
if (!function_exists('fb_flex_idx_property_detail_sc')) {
    function fb_flex_idx_property_detail_sc($atts, $content = null)
    {
        global $wp, $wpdb, $flex_idx_info, $flex_idx_lead;
...
if (!function_exists('fb_flex_idx_buildind_social_sc')) {
    function fb_flex_idx_buildind_social_sc()
    {
        ...
        $server_output = idxboost_remote_request(FLEX_IDX_API_BUILDING_LOOKUP_v2, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'body'    => $sendParams,
        ));
        $response = json_decode($server_output, true);
...
if (!function_exists('fb_flex_idx_sub_area_social_sc')) {
    function fb_flex_idx_sub_area_social_sc()
    {
        ...
        $server_output = idxboost_remote_request(FLEX_IDX_API_SUB_AREA_LOOKUP, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'body'    => $sendParams,
        ));
        $response = json_decode($server_output, true);
    ...
    */
    ...
    if ( !empty($page_slug) ) {
        ...
        if ('property' === $page_slug) {
            ...
            if (array_key_exists('imagens', $data)) {

                if (is_array($data['imagens']) && count($data['imagens']) > 0 ) {
                    $property_image_boo = preg_match('/(https)|(http)/', $data['imagens'][0]);
                    $property_image = $data['imagens'][0];

                    if (is_numeric($property_image_boo)) {
                        if ($property_image_boo == '0')
                            $property_image = 'http:' . $data['imagens'][0];
                    }                    
                }
            ...
            <meta property="og:title"
                  content="<?php bloginfo('name'); ?> - <?php echo $address_property . ' | ' . get_bloginfo(); ?>"/>
            <meta property="og:type" content="website"/>
            <meta property="og:url" content="<?php echo $domain_host; ?>"/>
            <meta property="og:description" content="<?php echo $addressPropertyDescription; ?>"/>
            <meta property="og:image" content="<?php echo $property_image; ?>"/>
...
if (!function_exists('flex_idx_property_detail_sc')) {
    function flex_idx_property_detail_sc($atts, $content = null)
    {
        ...
        if (empty($atts['registration_key'])) {
            ...
            list($slugname, $pagename, $slug) = $wp_request_exp;
        }
        ...
        global $agent_avatar_image;

        if (isset($agent_avatar_image)) {
        ...
        if (is_array($_GET) && count($_GET) > 0 && array_key_exists("vr", $_GET) && $_GET["vr"] == "1") {
            ...
            $response = idxboost_remote_request(FLEX_IDX_BASE_URL . "/rentals_listings/{$slug}", array(
                    'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                    'redirection' => 10,
                    'httpversion' => '1.1',
                    'body'    => array(
            ...
            $lookup_args = array(
                    'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                    'body'    => ($idx_v == "1") ? json_encode($sendParamsnewSearch) : http_build_query($sendParams),
            );

            if ($idx_v == "1") {
                $lookup_args['headers'] = array(
            ...
            $server_output = idxboost_remote_request(($idx_v == "1") ? FLEX_IDX_API_PROPERTY_DETAIL_V2 : FLEX_IDX_API_LOOKUP, $lookup_args);
            ...
            if (empty($property) && $type_lookup != 'sold') {
                ...
                $lookup_args_sold = array(
                        'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                        'body'    => ($idx_v == "1") ? json_encode(['for' => 'sold' ,'mls_num' => $mls_num, 'board_id' => $flex_idx_info['board_id'], 'extra_board_id' => $extra_boards_implode ]) : http_build_query($sendParams),
                );

                if ($idx_v == "1") {
                    $lookup_args_sold['headers'] = array(
                            'Content-Type'  => 'application/json',
                            'Authorization' => $access_token_service,
                    );
                } else {
                ...
                $server_output = idxboost_remote_request(($idx_v == "1") ? FLEX_IDX_API_PROPERTY_DETAIL_V2 : FLEX_IDX_API_LOOKUP, $lookup_args_sold);
                $response = json_decode($server_output, true);
            ...
            $registration_is_forced = (isset($agent_info["force_registration"]) && (true === $agent_info["force_registration"])) ? true : false;
                ...
                if($idx_v == "1"){
                    ...
                    $server_output_extra = idxboost_remote_request(FLEX_IDX_API_PROPERTY_DETAIL_EXTRA, array(
                            'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                            'redirection' => 10,
                            'httpversion' => '1.1',
                            'body'    => array(
...
if (!function_exists('flex_idx_filter_sc')) {
    function flex_idx_filter_sc($atts, $content = null)
    {
        ...
        if ($idx_v == "1") {
            ...
            }elseif ($atts["mode"] == "default") {
                assets_head_bundle_reactjs();
            }
            ...
            if ($atts['type'] == '2') {
                ...
                $access_token = flex_idx_get_access_token();
                $curlParams_body = idxboost_remote_request(FLEX_IDX_BASE_URL.'/info/exclusive', array(
                        'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                        'redirection' => 10,
                        'httpversion' => '1.1',
                        'headers' => array(
                ...
                $access_token = flex_idx_get_access_token();
                $curlParams_body = idxboost_remote_request(FLEX_IDX_BASE_URL.'/info/recentsale', array(
                        'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                        'redirection' => 10,
                        'httpversion' => '1.1',
                        'headers' => array(
                ...
                $access_token = flex_idx_get_access_token();
                $curlParams_body = idxboost_remote_request(FLEX_IDX_BASE_URL.'/info/displayfilters', array(
                        'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                        'redirection' => 10,
                        'httpversion' => '1.1',
                        'headers' => array(
                        ...
                        'body'    => 'filter_id='.$atts["id"].'&access_token='.$access_token,
                ));
                $responseParms = @json_decode($curlParams_body , true);
...
if (!function_exists('flex_idx_off_market_listings_sc')) {
    function flex_idx_off_market_listings_sc($atts)
    {
        ...
        $server_output = idxboost_remote_request(FLEX_IDX_API_LOOKUP_OFF_MARKET_LISTING, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'body'    => $sendParams,
        ));
        /*
...
if (!function_exists('idxboost_building_inventory_sc')) {
    function idxboost_building_inventory_sc($atts)
    {
        ...
        if ($atts["view"] == "slider") {
            $atts["action"] = "idxboost_collection_list";
            //$atts["building_id"] = "idxboost_collection_list";
            wp_localize_script('ib_slider_building_boost', 'filter_metadata_buiding', $atts);
        ...
        if ($atts["view"] == "slider") {
            if (file_exists(IDXBOOST_OVERRIDE_DIR . '/views/shortcode/idxboost_building_collection_slider.php')) {
                include IDXBOOST_OVERRIDE_DIR . '/views/shortcode/idxboost_building_collection_slider.php';
            } else {
                include FLEX_IDX_PATH . '/views/shortcode/idxboost_building_collection_slider.php';
            }
...
if (!function_exists('idx_buildind_group_sc')) {
    function idx_buildind_group_sc($atts)
    {
        ...
        $server_output = idxboost_remote_request(FLEX_IDX_API_BUILDING_GROUP_LOOKUP, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'body'    => $sendParams,
        ));
...
if (!function_exists('flex_idx_profile_sc')) {
    ...
    {
        global $wpdb, $flex_idx_info, $flex_idx_lead, $agent_registration_key, $agent_permalink;
...
if (!function_exists('flex_idx_favorites_sc')) {
    function flex_idx_favorites_sc($atts, $content = null)
    {
        ...
        $server_output = idxboost_remote_request(FLEX_IDX_API_FAVORITES_LIST, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'body'    => $sendParams,
        ));
...
if (!function_exists('flex_idx_saved_searches_sc')) {
    function flex_idx_saved_searches_sc($atts, $content = null)
    {
        ...
        $server_output = idxboost_remote_request(FLEX_IDX_API_SAVED_SEARCHES_LIST, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'body'    => $sendParams,
        ));
...
if (!function_exists('idx_download_neighborhood')) {
    function idx_download_neighborhood($id_package = '0', $registration_key = '0', $environment = 'staging')
    {
        ...
        $server_output = idxboost_remote_request(FLEX_IDX_BACKOFFICE_CPANEL_URL . '/tgapi/feed_automatic_neighborhood', array(
        ...
        $estruct_insert_page = "INSERT INTO {$wpdb->posts}(post_title,post_content,post_type,post_status,post_mime_type,post_name) values";
        $estruct_insert = "INSERT INTO {$wpdb->posts}(post_title,post_type,post_status,post_mime_type) values";
        $estruct_insert_silo = "INSERT INTO {$wpdb->posts}(post_title,post_type,post_status,post_mime_type) values";
        ...
        $result_item_neig = $wpdb->get_results("SELECT {$wpdb->posts}.ID as id_neig,post_relation.ID as id_page,{$wpdb->posts}.post_mime_type,post_relation.post_title FROM {$wpdb->posts} INNER JOIN {$wpdb->posts} as post_relation on post_relation.post_type='page' and post_relation.post_mime_type={$wpdb->posts}.post_mime_type where {$wpdb->posts}.post_type='neighborhood';");
        ...
        foreach ($items_silos as $value_add_silo) {
            foreach ($value_add_silo['data'] as $value_parent) {
                if ($value_add_silo['id_page_post'] != $value_parent['id']) {
                    $wpdb->query($wpdb->prepare("UPDATE {$wpdb->posts} SET post_parent = %d WHERE ID = %d", $value_add_silo['id_page_post'], $value_parent['id']));
                }
            }
...
if (!function_exists('idx_download_community')) {
    function idx_download_community($id_package = '0', $registration_key = '0', $environment = 'staging')
    {
        ...
        $current_user_id = get_current_user_id();
        $sendParams = array('id_package' => $id_package, 'registration_key' => $registration_key, 'environment' => $environment);
        $server_output = idxboost_remote_request(FLEX_IDX_BACKOFFICE_CPANEL_URL . '/tgapi/feed_automatic_community', array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_BULK,
                'idxboost_bulk' => true,
                'body'    => $sendParams,
        ));
        ...
        $estruct_insert_page = "INSERT INTO {$wpdb->posts}(post_title,post_content,post_type,post_status,post_mime_type,post_name,post_parent) values";
        $estruct_insert = "INSERT INTO {$wpdb->posts}(post_title,post_type,post_status,post_mime_type) values";
        $estruct_insert_silo = "INSERT INTO {$wpdb->posts}(post_title,post_type,post_status,post_mime_type) values";

        $wp_postmeta_insert = "INSERT INTO {$wpdb->postmeta}(post_id,meta_key,meta_value) values";

        $items_neighborhood = $wpdb->get_results("SELECT ID,post_title,post_mime_type,{$wpdb->postmeta}.meta_value as page_wp FROM {$wpdb->posts} left join {$wpdb->postmeta} on {$wpdb->postmeta}.post_id={$wpdb->posts}.ID and {$wpdb->postmeta}.meta_key='tgpost_relacion' where post_type='neighborhood' and post_status='publish';", ARRAY_A);
        ...
        $result_item_neig = $wpdb->get_results("SELECT {$wpdb->posts}.ID as id_neig,post_relation.ID as id_page,{$wpdb->posts}.post_mime_type,post_relation.post_title FROM {$wpdb->posts} INNER JOIN {$wpdb->posts} as post_relation on post_relation.post_type='page' and post_relation.post_mime_type={$wpdb->posts}.post_mime_type where {$wpdb->posts}.post_type='communities';");
...
if (!function_exists('idx_download_building')) {
    function idx_download_building($id_package = '0', $registration_key = '0', $environment = 'staging')
    {
        ...
        $sendParams = array('id_package' => $id_package, 'registration_key' => $registration_key, 'environment' => $environment);
        $server_output = idxboost_remote_request(FLEX_IDX_BACKOFFICE_CPANEL_URL . '/tgapi/feed_automatic_building', array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_BULK,
                'idxboost_bulk' => true,
                'body'    => $sendParams,
        ));
        ...
        $result_pages_build_wp = $wpdb->get_results("SELECT {$wpdb->posts}.ID,{$wpdb->posts}.post_title,{$wpdb->posts}.post_mime_type FROM {$wpdb->posts} where {$wpdb->posts}.post_type='tgbuilding' and post_status='publish';");
        $items_neighborhood = $wpdb->get_results("SELECT ID,post_title,post_mime_type FROM {$wpdb->posts} where post_type='neighborhood' and post_status='publish';", ARRAY_A);
        $items_communities = $wpdb->get_results("SELECT ID,post_title,post_mime_type FROM {$wpdb->posts} where post_type='communities' and post_status='publish';", ARRAY_A);
        ...
        $result_idxboost_build_pa = $wpdb->get_results("SELECT {$wpdb->posts}.ID,{$wpdb->posts}.post_title,{$wpdb->posts}.post_mime_type FROM {$wpdb->posts} where {$wpdb->posts}.post_type='flex-idx-building' and post_status='publish';");
...
if (!function_exists('idxboost_cms_page_under_construction')) {
    function idxboost_cms_page_under_construction()
    {
        status_header(404);
        nocache_headers();

        if (file_exists(IDXBOOST_OVERRIDE_DIR . '/views/pages/under-construction-page-template.php')) {
            include IDXBOOST_OVERRIDE_DIR . '/views/pages/under-construction-page-template.php';
        } else {
            include FLEX_IDX_PATH . '/views/pages/under-construction-page-template.php';
        }
    }
}

if (!function_exists('idxboost_cms_page_under_maintenance')) {
    function idxboost_cms_page_under_maintenance()
    {
        status_header(404);
        nocache_headers();

        if (file_exists(IDXBOOST_OVERRIDE_DIR . '/views/pages/under-maintenance-page-template.php')) {
            include IDXBOOST_OVERRIDE_DIR . '/views/pages/under-maintenance-page-template.php';
        } else {
            include FLEX_IDX_PATH . '/views/pages/under-maintenance-page-template.php';
        }
    }
}
...
if (!function_exists('flex_idx_our_agents_sc')) {
    function flex_idx_our_agents_sc($atts, $content = null)
    {
        ...
        $curl_body = idxboost_remote_request($service_agent_list, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'redirection' => 10,
                'httpversion' => '1.1',
                'headers' => array(
                ...
                'body'    => json_encode(["category" => $atts["category"]]),
        ));
        $response = @json_decode($curl_body, true);
...
if (!function_exists('flex_idx_detail_agents_sc')) {
    function flex_idx_detail_agents_sc($atts, $content = null)
    {
        ...
        $curl_body = idxboost_remote_request($service_agent_detail, array(
                'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                'redirection' => 10,
                'httpversion' => '1.1',
                'headers' => array(
...
if (!function_exists('idxboost_new_search_filters_sc')) {
    function idxboost_new_search_filters_sc($atts, $content = null)
    {
        ...
        if ( !empty($atts["filter_id"]) ) {
            ...
            $access_token = flex_idx_get_access_token();
            $curlParams_body = idxboost_remote_request(FLEX_IDX_BASE_URL.'/get/map_search_filter/'.$atts["filter_id"], array(
                    'timeout' => IDXBOOST_HTTP_TIMEOUT_RENDER,
                    'redirection' => 10,
                    'httpversion' => '1.1',
                    'headers' => array(
...
if (!function_exists('dashtodash_sc')) {
    function dashtodash_sc($atts, $content = null)
    {
        global $flex_idx_info,$wpdb;
        ...
        if (file_exists(IDXBOOST_OVERRIDE_DIR . '/views/shortcode/dashtodash_sc.php')) {
                include IDXBOOST_OVERRIDE_DIR . '/views/shortcode/dashtodash_sc.php';
        } else {
                include FLEX_IDX_PATH . '/views/shortcode/dashtodash_sc.php';
        }            
