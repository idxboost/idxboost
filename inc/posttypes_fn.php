...
function function_idx_agent_page($single_template)
{
    ...
    //     'agent_slug_name' => $agent_slug_name,
    //     'agent_page_name' => $agent_page_name,
    //     'flex_agent_id' => $flex_agent_id,
    //     'flex_agent_slug' => $flex_agent_slug,
    //     'flex_agent_registration_key' => $flex_agent_registration_key
    // ]);
    // echo '</pre>';
...
<?php
    ...
    if (!function_exists('flex_filter_page_fl_cb')) {
        function flex_filter_page_fl_cb($post)
        {
            ...
            ?>
            <div class="inside">
                <p><input type="radio" name="_flex_filter_page_fl" <?php checked($flex_filter_page_fl, 3);?> value="3"> Default</p>
                <p><input type="radio" name="_flex_filter_page_fl" <?php checked($flex_filter_page_fl, 1);?> value="1"> Recent Sales</p>
                <p><input type="radio" name="_flex_filter_page_fl" <?php checked($flex_filter_page_fl, 2);?> value="2"> Exclusive Listings</p>
                <p><input type="radio" name="_flex_filter_page_fl" <?php checked($flex_filter_page_fl, 0);?> value="0"> Hide Search Bar</p>
            </div>
            <?php
}
    }
    ...
    if (!function_exists('flex_filter_page_references_fl_cb')) {
        function flex_filter_page_references_fl_cb($post)
        {
            ...
            ?>
            <input type="checkbox" name="_flex_filter_page_references" value="1" <?php if ( !empty($flex_filter_page_refrences) && $flex_filter_page_refrences=="1" ) { echo "checked"; } ?> > Active 
            <?php
}
    }    
...
function func_idx_style($post)
{
    ...
    ?>
<link rel="stylesheet" href="<?php echo FLEX_IDX_URI; ?>/css/color-picker.min.css" /> <script src="<?php echo FLEX_IDX_URI; ?>/js/color-picker.min.js"></script>
...
<style type="text/css">.change_button, .change_button_default { display: inline-block;background-color: gray; border-radius: 20px; width: 112px; text-align: center; color: white; padding: 5px; cursor: pointer; } .item_object input, .item_object .change_button { display: inline-block; } .item_object { margin-bottom: 20px; } .change_button:hover, .button_on, .change_button_default:hover {background-color: #403131; } div#button_default { width: 190px; right: 0px; position: relative; }</style>
...
jQuery('#idx_style_search_bar_bose, input[name=idx_select_image]').change(function(){ func_preview(); });

function func_preview(){
        var status_search='';var style_search_bar_texto='';var style_body_point_status_search='';var style_boton_point='';var style_texto_point='';var select_image_point='';
        if( jQuery('#idx_style_search_bar_bose').val()=='transparent') status_search=hexToRgbNew(jQuery('#idx_style_5').val()); else status_search=jQuery('#idx_style_5').val();
...
jQuery('.change_button').click(function() {
    ...
    picker.on("change", function(color) { this.target.value = "#" + color;  });
...
';
    ...
    if (strlen((wp_get_attachment_url(get_option('footer_media_first_id')))) > 0) {$image_footer_firt = 'li.logo-footer img { content: url("' . wp_get_attachment_url(get_option('footer_media_first_id')) . '"); }';} else { $image_footer_firt = '';}
    if (strlen((wp_get_attachment_url(get_option('footer_media_second_id')))) > 0) {$image_footer_second = 'li.company-parnet img.realtor_footer { content: url("' . wp_get_attachment_url(get_option('footer_media_second_id')) . '"); }';} else { $image_footer_second = '';}
    if (strlen((wp_get_attachment_url(get_option('footer_media_third')))) > 0) {$image_footer_third = 'li.company-parnet img.realtor_footer_2 { content: url("' . wp_get_attachment_url(get_option('footer_media_third')) . '"); }';} else { $image_footer_third = '';}
    ...
    if (strlen(($idx_style_texto_theme)) > 0) {$idx_style_texto_theme_point = '#header .wrap-options #user-options>li.login a, #header .wrap-options #user-options>li.register a, #header .wrap-menu nav ul li a { color: ' . $idx_style_texto_theme . '; } ';}
    if (strlen(($idx_style_body_text_theme)) > 0) {$idx_style_body_text_theme_point = '.flex-block-description, .flex-site-description li, .flex-footer-content .gwr .flex-company-site li.submenu-site .submenu-footer li a, .flex-footer-content .gwr .flex-company-site li { color: ' . $idx_style_body_text_theme . ' !important; } ';}
...
<style type="text/css">.item_object img { max-width: 150px; height: auto; } .change_button, .change_button_default { display: inline-block;background-color: gray; border-radius: 20px; width: 112px; text-align: center; color: white; padding: 5px; cursor: pointer; } .item_object input, .item_object .change_button { display: inline-block; } .item_object { margin-bottom: 20px; } .change_button:hover, .button_on, .change_button_default:hover {background-color: #403131; } div#button_default { width: 190px; right: 0px; position: relative; }</style>
<script type="text/javascript">
function hexToRgbNew(hex) {
    hex=hex.replace('#','');
  var arrBuff = new ArrayBuffer(4);
...
jQuery('#idx_style_search_bar_bose_theme, input[name=idx_select_image]').change(function(){ func_preview(); });

function func_preview(){
        var status_search='';var style_body_point_status_search='';var style_boton_point='';var style_texto_point='';var style_title_menu=''; var style_title_galery='';
        var image_footer_firt=''; var image_footer_second=''; var image_footer_third='';
        ...
        if( jQuery('#idx_style_search_bar_bose_theme').val()=='transparent') status_search=hexToRgbNew(jQuery('#idx_style_5').val()); else status_search=jQuery('#idx_style_5').val();

        if (jQuery('#image-preview-first').attr('src').length > 0) {    image_footer_firt='li.logo-footer img { content: url("'+jQuery('#image-preview-first').attr('src')+'"); }';     }else {image_footer_firt='';}
        if (jQuery('#image-preview-second').attr('src').length > 0) {   image_footer_second ='li.company-parnet img.realtor_footer { content: url("'+jQuery('#image-preview-second').attr('src')+'"); }';   }else {image_footer_second='';}
        if (jQuery('#image-preview-third').attr('src').length >0 ) {    image_footer_third ='li.company-parnet img.realtor_footer_2 { content: url("'+jQuery('#image-preview-third').attr('src')+'"); }';   }else{ image_footer_third='';}
        ...
        if ( (jQuery('.thickbox img').attr('src')) !=undefined ) style_title_galery='#header .wrap-menu .logo-content img.logo { content: url("'+jQuery('.thickbox img').attr('src')+'"); }'; else style_title_galery='#header .wrap-menu .logo-content img.logo { content: url("<?php echo FLEX_IDX_URI; ?>preview_plugin/images/logo.png"); }';
        //if ((jQuery('.idx_style_search_bar_texto').length)>0 ) style_search_bar_texto='#flex-bubble-search input{ color: '+jQuery('.idx_style_search_bar_texto').val()+'; } ';
...
jQuery('#_thumbnail_id').change( function(){ console.log(jQuery('.thickbox img').attr('src')); });
...
<style type="text/css">.item_complements p {line-height: 3.5;}.item_complements div { font-weight: bold; margin-bottom: 8px; }.item_complements { margin-bottom: 15px; }span.remove:hover { background-color: #900c0c; }.item_complements input { width: 80%; height: 39px; padding-left: 15px;} span.remove { padding: 11px; background-color: red; color: white; cursor: pointer; }
@media screen and (max-width: 680px) {.item_complements input { width: 100%; } }
</style>
<script>
