...
if (!function_exists('flex_lead_signup_xhr_fn')) {
    function flex_lead_signup_xhr_fn()
    {
        ...
        $invite_token = isset($_COOKIE['invite_token']) ? trim(strip_tags($_COOKIE['invite_token'])) : '';
        ...
        if (!empty($registration_key)) {
            ...
            $agent_slug = $wpdb->get_var(
                    "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = '_flex_agent_registration_key' AND meta_value = '{$registration_key}'"
            );
        }
                ...
                <?php if (!empty($my_flex_pages)) : ?>
                    <ul>
                        <?php foreach ($my_flex_pages as $my_flex_page) : ?>
                            ...
                            <li>
                                <a href="<?php echo (true === $is_agent_page) ? str_replace(site_url(), $agent_slug, $my_flex_page['permalink']) : $my_flex_page['permalink']; ?>"><?php echo $my_flex_page['post_title']; ?></a>
                            </li>
                        <?php endforeach; ?>
                        <li>
    ...
    function flex_idx_connect_fn($return_json = false)
    {
        ...
        if (is_array($response['slugs_settings']) && count($response['slugs_settings']) > 0) {
            ...
            if (array_key_exists("property", $response['slugs_settings'])) {
                ...
                $wpdb->query("UPDATE {$wpdb->posts} set post_name='{$Siteproperty}',guid='{$wpsite}/{$Siteproperty}' where post_type='flex-idx-pages' and post_content = '[flex_idx_property_detail]'; ");
            }

            if (array_key_exists("building", $response['slugs_settings'])) {
                $Sitebuilding = $response['slugs_settings']["building"];
                $wpdb->query("UPDATE {$wpdb->posts} set post_name='{$Sitebuilding}', guid='{$wpsite}/{$Sitebuilding}' where post_type='flex-idx-pages' and post_title = 'building'; ");
            }

            if (array_key_exists("search", $response['slugs_settings'])) {
                $Sitesearch = $response['slugs_settings']["search"];
                $wpdb->query("UPDATE {$wpdb->posts} set post_name='{$Sitesearch}',guid='{$wpsite}/{$Sitesearch}' where post_type='flex-idx-pages' and post_content = '[ib_search]'; ");
            }
        ...
        if (isset($response['success']) && $response['success'] === true) {
            ...
            */
            ...
            if (!empty($response['search_filter_settings'])) {
                $search_filter_settings = $response['search_filter_settings'];
                update_option('idxboost_search_filter_settings', $search_filter_settings);
            }
...
if (!function_exists('flex_idx_register_assets')) {
    function flex_idx_register_assets()
    {
        ...
        $board_info = [];
        if (is_array($flex_idx_info) && array_key_exists("board_info", $flex_idx_info)) {
            $board_info = $flex_idx_info["board_info"];
        }
        ...
        $custom_strings = array(
            ...
            // 'open_house' => __('Open House', IDXBOOST_DOMAIN_THEME_LANG),
                ...
                'enter_a_phone_number' => __('Phone Number', IDXBOOST_DOMAIN_THEME_LANG),
        );
        // main styles
        ...
        wp_localize_script('flex-idx-search-commercial-v2', '__flex_idx_search_filter_v2', array(
                ...
                'lookupListingsDetail' => ($idx_v == "1") ? FLEX_IDX_API_PROPERTY_DETAIL_V2 : FLEX_IDX_API_SEARCH_LISTING,
                'access_token_search' => $access_token_service,
...
//        }
        ...
        wp_localize_script('ib_slider_filter', 'idx_param_slider', array(
                'endpoint_v3' => FLEX_IDX_DISPLAY_FILTER_V3,
                'endpoint_v2' => FLEX_IDX_DISPLAY_FILTER_V2_old,
                'endpoint_colletion_v3' => FLEX_IDX_API_MARKET_RECENT_SALE_v2,
                'endpoint_v2_exclusive' => FLEX_IDX_API_MARKET_EXCLUSIVE_LISTINGS,
                'endpoint_v3_exclusive' => FLEX_IDX_API_MARKET_EXCLUSIVE_LISTINGS_v2,
                'endpoint_v3_sold_listing' => FLEX_IDX_API_MARKET_RECENT_SALE_v2,
...
function insert_assets_head_new_collections_properties()
{
    ...
    if (has_shortcode($content, 'new_collections_properties')) {
        ?>
        <script type="module" crossorigin
                src="<?php echo FLEX_IDX_URI . 'react/collections/bundle.js?ver=' . iboost_get_mod_time("react/collections/bundle.js"); ?>"></script>
        <link rel="stylesheet"
              href="<?php echo FLEX_IDX_URI . 'react/collections/bundle.css?ver=' . iboost_get_mod_time("react/collections/bundle.css"); ?>"/>
        <?php
    }
}
...
function insert_assets_head_new_collections_properties_details()
{
    ...
    if (has_shortcode($content, 'new_collections_properties_details')) {
        ?>
        <script type="module" crossorigin
                src="<?php echo FLEX_IDX_URI . 'react/collections_detail/assets/bundle.js?ver=' . iboost_get_mod_time("react/collections_detail/assets/bundle.js"); ?>"></script>
        <script async
                src="<?php echo sprintf('//maps.googleapis.com/maps/api/js?libraries=drawing,geometry,marker&key=%s&callback=Function.prototype', $flex_idx_info["agent"]["google_maps_api_key"]) ?>"></script>
        <link rel="stylesheet"
              href="<?php echo FLEX_IDX_URI . 'react/collections_detail/assets/bundle.css?ver=' . iboost_get_mod_time("react/collections_detail/assets/bundle.css"); ?>"/>
        <link rel="stylesheet"
              href="<?php echo FLEX_IDX_URI . 'react/collections_detail/fonts/icons/style.min.css?ver=' . iboost_get_mod_time("react/collections_detail/fonts/icons/style.min.css"); ?>"/>
        <?php
    }
}
...
function insert_assets_head_dashtodash_sc()
{
    ...
    if (has_shortcode($content, 'dashtodash')) {
        ?>
        <script type="module" crossorigin
                src="<?php echo FLEX_IDX_URI . 'react/dash-to-dash/assets/bundle.js?ver=' . iboost_get_mod_time("react/dash-to-dash/assets/bundle.js"); ?>"></script>
        <script async
                src="<?php echo sprintf('//maps.googleapis.com/maps/api/js?libraries=drawing,geometry,marker&key=%s&callback=Function.prototype', $flex_idx_info["agent"]["google_maps_api_key"]) ?>"></script>
        <link rel="stylesheet"
              href="<?php echo FLEX_IDX_URI . 'react/dash-to-dash/fonts/icons/style.css?ver=' . iboost_get_mod_time("react/dash-to-dash/fonts/icons/style.css"); ?>"/>
        <link rel="stylesheet"
              href="<?php echo FLEX_IDX_URI . 'react/dash-to-dash/assets/bundle.css?ver=' . iboost_get_mod_time("react/dash-to-dash/assets/bundle.css"); ?>"/>
        <?php
    }
}
...
if (!function_exists('greatsliderLoad')) {
    function greatsliderLoad()
    {
        ...
        wp_enqueue_script('flex-print-area', FLEX_IDX_URI . 'js/vendor/jquery.print-area.min.js', array('flex-cookies-manager', 'jquery'), iboost_get_mod_time("js/vendor/jquery.print-area.min.js"), true);
...
if (!function_exists('flex_idx_profile_extend_fn')) {
    function flex_idx_profile_extend_fn($user)
    {
        ...
        <table class="form-table">
            <tr>
                <th>
                    <label for="flex_idx_profile_phone"><?php echo __('Phone Number', IDXBOOST_DOMAIN_THEME_LANG); ?></label>
                </th>
                <td>
            ...
            <tr>
                <th><label for="flex_idx_profile_city"><?php echo __('City', IDXBOOST_DOMAIN_THEME_LANG); ?></label>
                </th>
                <td>
            ...
            <tr>
                <th><label for="flex_idx_profile_state"><?php echo __('State', IDXBOOST_DOMAIN_THEME_LANG); ?></label>
                </th>
                <td>
            ...
            <tr>
                <th><label for="flex_idx_profile_zip"><?php echo __('Zip Code', IDXBOOST_DOMAIN_THEME_LANG); ?></label>
                </th>
                <td>
...
if (!function_exists('idxboost_cms_register_assets')) {
    function idxboost_cms_register_assets()
    {
        ...
        if (
            ...
            wp_register_style(
                    'carbonite',
                    get_cms_assets_base_url() . '/assets/css/carbonite.css',
                    array(
...
if (!function_exists('idxboost_cms_get_menu')) {
    ...
    {
        global $flex_idx_info, $idxboost_cms_menu;
...
if (!function_exists('idxboost_cms_get_header_footer')) {
    ...
    {
        global $flex_idx_info, $idxboost_cms_header_footer;
...
if (!function_exists('idxboost_cms_remove_default_dns_prefetch')) {
    function idxboost_cms_remove_default_dns_prefetch($urls, $relation_type)
    {
        if ($relation_type === 'dns-prefetch') {
            $urls = array_filter($urls, function ($url) {
                return !in_array($url, [
                        'api-cms.idxboost.dev',
                        'api-cms.idxboost.com',
                        'idxboost-spw-assets.idxboost.us',
                ], true);
            });
        }
...
if (!function_exists('idx_autologin_collections')) {
    function idx_autologin_collections()
    {
        if (isset($_GET['secureToken']) && strpos($_SERVER["REQUEST_URI"], 'secureToken=') !== false) {

            $token = $_GET['secureToken'];

            // --- DECODIFICACIÓN MANUAL (Ya verificamos que esto funciona) ---
            $parts = explode('.', $token);
            if (count($parts) < 2) return;
            $payloadBase64 = str_replace(['-', '_'], ['+', '/'], $parts[1]);
            $remainder = strlen($payloadBase64) % 4;
            if ($remainder) $payloadBase64 .= str_repeat('=', 4 - $remainder);
            $decoded_array = json_decode(base64_decode($payloadBase64), true);
            if (!$decoded_array) return;
            ...
            // Nombres
            $firstName = $decoded_array['firstName'] ?? ($decoded_array['first_name'] ?? '');
            $lastName  = $decoded_array['lastName'] ?? ($decoded_array['last_name'] ?? '');

            // Teléfonos (Agregado para igualar al JS)
            $phone = $decoded_array['phone'] ?? ($decoded_array['phone_number'] ?? '');
            $phoneCode = $decoded_array['phone_code'] ?? '';

            if (empty($firstName) && !empty($email)) {
                $parts_email = explode('@', $email);
                $firstName = $parts_email[0];
            }

            // ---------------------------------------------------------
            // CONFIGURACIÓN DE COOKIES (LA CLAVE DEL ARREGLO)
            // ---------------------------------------------------------
            $expire = time() + (30 * 86400);
            $path = '/';
            $domain = ""; // Dejar vacío suele funcionar mejor que poner el host manualmente en local/staging
            $secure = is_ssl();
            $httponly = false; // VITAL: False

            // 1. EL TOKEN: Usamos setrawcookie para que PHP NO lo modifique (no url-encode)
            setrawcookie('ib_lead_token', $token, $expire, $path, $domain, $secure, $httponly);

            // 2. DATOS DE USUARIO: Usamos setcookie normal (estos sí pueden tener espacios o tildes)
            setcookie('_ib_user_email', $email, $expire, $path, $domain, $secure, $httponly);
            setcookie('_ib_user_firstname', $firstName, $expire, $path, $domain, $secure, $httponly);
            setcookie('_ib_user_lastname', $lastName, $expire, $path, $domain, $secure, $httponly);

            // 3. DATOS DE TELÉFONO (Tu JS los guarda, así que PHP también debe hacerlo)
            setcookie('_ib_user_phone', $phone, $expire, $path, $domain, $secure, $httponly);
            setcookie('_ib_user_code_phone', $phoneCode, $expire, $path, $domain, $secure, $httponly);

            // 4. PERSISTENCIA
            setcookie('_ib_last_logged_in_username', $email, $expire, $path, $domain, $secure, $httponly);
            setcookie('social_register', 'activo', $expire, $path, $domain, $secure, $httponly);

            ?>
            <script type="text/javascript">
                <?php if (!empty($idx_info_lead)) { ?>
                let idx_info_lead = <?php echo json_encode($decoded_array); ?>;

                (function ($) {
                    //INICITIAL VARIABLES USER
                    $("#_ib_fn_inq").val(idx_info_lead.first_name);
                    $("#_ib_ln_inq").val(idx_info_lead.last_name);
                    $("#_ib_em_inq").val(idx_info_lead.email);
                    $("#_ib_ph_inq").val(idx_info_lead.phone_number);
                    ...
                    if (ob_form_building_footer.length > 0) {
                        ...
                        ob_form_building_footer.find('[name="last_name"]').val(idx_info_lead.last_name);
                        ob_form_building_footer.find('[name="email"]').val(idx_info_lead.phone_number);
                        ob_form_building_footer.find('[name="phone"]').val(idx_info_lead.email);
                    }
                    ...
                    if (ob_form_modal.length > 0) {
                        ...
                        ob_form_modal.find('[name="last_name"]').val(idx_info_lead.last_name);
                        ob_form_modal.find('[name="email_address"]').val(idx_info_lead.phone_number);
                        ob_form_modal.find('[name="phone_number"]').val(idx_info_lead.email);
                    }
                    ...
                    if (ob_form_off_market_listing.length > 0) {
                        ...
                        ob_form_off_market_listing.find('[name="last_name"]').val(idx_info_lead.last_name);
                        ob_form_off_market_listing.find('[name="email"]').val(idx_info_lead.phone_number);
                        ob_form_off_market_listing.find('[name="phone"]').val(idx_info_lead.email);
                    }
                    //INICITIAL VARIABLES USER
            ...
            if (!empty($mls)) {
                ...
                $redirect_url .= $separator . 'show=' . $mls;
            }
...
if (!function_exists('idx_autologin_authenticate')) {
    ...
    {
        if (strpos($_SERVER["REQUEST_URI"], '/idxboostLoginAdmin?token=') !== false) {
            $publicKey = <<<EOD
            -----BEGIN PUBLIC KEY-----
            ...
            try {
                ...
                $from = $decoded_array['from'];
                $fromUser = $decoded_array['from_user'];
                ...
                if ($decoded_array['exp'] >= $issuedAt->getTimestamp()) {
                    if ($userToLogin) {
                        ...
                        $sendParams = [
                                ...
                                "action" => "Login (WordPress)",
                                "object" => "Memberships",
                                "information" => get_bloginfo('url'),
                                "ip" => $_SERVER['REMOTE_ADDR'],
                                "details" => json_encode([
                                        "Installation" => get_bloginfo('url'),
                                        "Page" => $page,
                                        "From" => $from,
                                        "User" => $fromUser
                                ])
...
if (!function_exists('flex_idx_get_email_by_invite_token')) {
    function flex_idx_get_email_by_invite_token()
    {
        // 1. Verificar si el token está presente
        $invite_token = isset($_POST['invite_token']) ? sanitize_text_field($_POST['invite_token']) : '';

        if (empty($invite_token)) {
            wp_send_json_error(array('message' => 'Token de invitación no proporcionado.'));
            wp_die();
        }

        //cambiar a configuracion
        $api_base_url = FLEX_IDX_API_COLLECTIONS . '/api/collections';
        $api_endpoint = $api_base_url . '/get-invitation/' . $invite_token;


        $response = wp_remote_get($api_endpoint);

        if (is_wp_error($response)) {
            wp_send_json_error(array('message' => 'Error al conectar con la API externa.', 'details' => $response->get_error_message()));
            wp_die();
        }
        ...
        // 4. Procesar la respuesta de la API externa
        if (isset($data['data']['email'])) {
            // La API externa devolvió el email
            wp_send_json_success(array('email' => $data['data']['email']));
        } else {
            // Error de la API externa o no se encontró el email
            wp_send_json_error(array('message' => 'Token inválido o no se encontró el email.'));
        }
        wp_die();
    }
}
...
function insert_assets_head_dinamic_forms_sell_rent_buy()
{
    global $post;

    $content = $post->post_content;

    if (has_shortcode($content, 'idx_sell_rent_dinamic_forms')) {

        idxboost_print_vite_assets([
                'distDir' => ib_get_idx_path() . 'react/sell_rent_dinamic_forms/dist/',
                'distUrl' => FLEX_IDX_URI . 'react/sell_rent_dinamic_forms/dist/',
                'iconCssUrl' => FLEX_IDX_URI . 'react/sell_rent_dinamic_forms/fonts/icons/style.min.css?ver=' .
                        iboost_get_mod_time('react/sell_rent_dinamic_forms/fonts/icons/style.min.css')
        ]);
